...
trait DatabaseModuleTestOps[Connection]
   ...
   */
  def noConnection(implicit trace: Trace): ZIO[Any, Nothing, Connection] = connectionFromJdbc(
    NoopJdbcConnection
  )
      ...
      new Service {
        ...
        override def transaction[R, E, A](
          ...
          noConnection.flatMap { c =>
            ZIO
              .environmentWith[R](_ ++ ZEnvironment(c))
              .flatMap(zio.provideEnvironment(_))
              .mapError(Right(_))
          }

        override def autoCommit[R, E, A](zio: => ZIO[Connection with R, E, A])(implicit
          errorStrategies: ErrorStrategiesRef,
          trace: Trace
        ): ZIO[R, Either[DbException, E], A] =
          noConnection.flatMap { c =>
