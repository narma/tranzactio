...
abstract class DatabaseModuleBase[Connection, Database <: DatabaseOps.ServiceOps[Connection]: Tag]
  ...
  override def transaction[R, E, A](
    ...
    trace: Trace
  ): ZIO[Database with R, Either[DbException, E], A] =
  ...
  def connectionFromJdbc(connection: => JdbcConnection)(implicit
    trace: Trace
   ...
   */
  final def fromDatasource(errorStrategies: ErrorStrategiesRef)(implicit
    trace: Trace
  ): ZLayer[DataSource, Nothing, Database] =
  ...
  final def fromDatasourceAndErrorStrategies(implicit
    trace: Trace
  ): ZLayer[DataSource with ErrorStrategies, Nothing, Database] =
