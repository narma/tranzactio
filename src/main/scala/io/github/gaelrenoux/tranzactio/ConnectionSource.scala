...
object ConnectionSource {
  ...
  abstract class ServiceBase(
    ...
    def runTransaction[R, E, A](
      ...
      ZIO.acquireReleaseWith(openConnection.mapError(Left(_)))(closeConnection(_).orDie) {
        (c: Connection) =>
          setAutoCommit(c, autoCommit = false)
            ...
            .tapBoth(
              _ =>
                if (commitOnFailure) commitConnection(c).mapError(Left(_))
                else rollbackConnection(c).mapError(Left(_)),
              _ => commitConnection(c).mapError(Left(_))
            )
      }

    def runAutoCommit[R, E, A](task: Connection => ZIO[R, E, A])(implicit
      errorStrategies: ErrorStrategiesRef,
      trace: Trace
    ): ZIO[R, Either[DbException, E], A] =
      ZIO.acquireReleaseWith(openConnection.mapError(Left(_)))(closeConnection(_).orDie) {
    ...
    def openConnection(implicit
      errorStrategies: ErrorStrategiesRef,
      trace: Trace
    ): ZIO[Any, DbException, Connection] =
      wrap(bottomErrorStrategy.openConnection) {
        getConnection.mapError(e => DbException.Wrapped(e))
      }

    def setAutoCommit(c: => Connection, autoCommit: => Boolean)(implicit
      errorStrategies: ErrorStrategiesRef,
      trace: Trace
    ): ZIO[Any, DbException, Unit] =
    ...
    private def wrap[R, A](es: ErrorStrategy)(z: => ZIO[Any, Throwable, A])(implicit trace: Trace) =
      es {
  ...
  private class SingleConnectionService(
    ...
    override def closeConnection(
      c: => Connection
    )(implicit errorStrategies: ErrorStrategiesRef, trace: Trace): ZIO[Any, Nothing, Unit] =
      ZIO.unit

    override def runTransaction[R, E, A](
      task: Connection => ZIO[R, E, A],
    ...
    override def runAutoCommit[R, E, A](task: Connection => ZIO[R, E, A])(implicit
      errorStrategies: ErrorStrategiesRef,
      trace: Trace
    ): ZIO[R, Either[DbException, E], A] =
      semaphore.withPermit {
  ...
  def fromDatasourceAndErrorStrategies(implicit
    trace: Trace
  ): ZLayer[DataSource with ErrorStrategies, Nothing, ConnectionSource] =
    ZLayer {
      for {
        source          <- ZIO.service[DataSource]
   ...
   */
  def fromConnection(
    errorStrategiesRef: ErrorStrategiesRef
  )(implicit trace: Trace): ZLayer[Connection, Nothing, ConnectionSource] =
    ZLayer {
      for {
