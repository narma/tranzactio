...
package object doobie extends Wrapper {
  ...
  final def tzioStream[A](q: => fs2.Stream[Query, A], queueSize: => Int = DefaultStreamQueueSize)(
    implicit trace: Trace
  ): TranzactIOStream[A] =
    ZStream
      .serviceWithStream[Connection] { c =>
  ...
  object Database
    ...
    override final def connectionFromJdbc(
      connection: => JdbcConnection
    )(implicit trace: Trace): ZIO[Any, Nothing, Connection] =
      ZIO.succeed {
    ...
    override final def fromConnectionSource(implicit
      trace: Trace
    ): ZLayer[ConnectionSource, Nothing, Database] =
      ZLayer.fromFunction { (cs: ConnectionSource) =>
        new DatabaseServiceBase[Connection](cs) {
          override final def connectionFromJdbc(connection: => JdbcConnection)(implicit
            trace: Trace
          ): ZIO[Any, Nothing, Connection] =
