...
object DoobieIT extends ITSpec {
  ...
  val connectionCountQuery: TranzactIO[Int] = tzio(
    Fragment.const(connectionCountSql).query[Int].unique
  )
  ...
  private val testDataCommittedOnTransactionSuccess: MySpec =
    test("data committed on transaction success") {
      wrap {
        for {
  ...
  private val testConnectionClosedOnTransactionSuccess: MySpec =
    test("connection closed on transaction success") {
      wrap {
        for {
          _               <- Database.transaction(PersonQueries.setup)
          _               <- Database.transaction(PersonQueries.insert(buffy))
  ...
  private val testDataRollbackedOnTransactionFailure: MySpec =
    test("data rollbacked on transaction failure if commitOnFailure=false") {
      wrap {
        for {
          _       <- Database.transaction(PersonQueries.setup)
  ...
  private val testDataCommittedOnTransactionFailure: MySpec =
    test("data committed on transaction failure if commitOnFailure=true") {
      wrap {
        for {
          _ <- Database.transaction(PersonQueries.setup)
          _ <- Database
                 .transaction(
                   PersonQueries.insert(buffy) <*> PersonQueries.failing,
  ...
  private val testConnectionClosedOnTransactionFailure: MySpec =
    test("connection closed on transaction failure") {
      wrap {
        for {
          _               <- Database.transaction(PersonQueries.setup)
          _               <- Database.transaction(PersonQueries.insert(buffy) <*> PersonQueries.failing).flip
  ...
  private val testDataCommittedOnAutoCommitSuccess: MySpec =
    test("data committed on autoCommit success") {
      wrap {
        for {
  ...
  private val testConnectionClosedOnAutoCommitSuccess: MySpec =
    test("connection closed on autoCommit success") {
      wrap {
        for {
          _               <- Database.autoCommit(PersonQueries.setup)
          _               <- Database.autoCommit(PersonQueries.insert(buffy))
  ...
  private val testDataRollbackedOnAutoCommitFailure: MySpec =
    test("data rollbacked on autoCommit failure") {
      wrap {
        for {
          _       <- Database.autoCommit(PersonQueries.setup)
  ...
  private val testConnectionClosedOnAutoCommitFailure: MySpec =
    test("connection closed on autoCommit failure") {
      wrap {
        for {
          _               <- Database.autoCommit(PersonQueries.setup)
          _               <- Database.autoCommit(PersonQueries.insert(buffy))
    ...
    wrap {
      for {
        ...
        result <- Database.autoCommit {
                    val doobieStream = sql"""SELECT given_name, family_name FROM person"""
                      .query[Person]
                      .streamWithChunkSize(1) // make sure it's read one by one
                      .map { p =>
                        if (p.givenName == "Rupert")
                          throw new IllegalStateException // fail on the second one, if it's ever read
                        else p
                      }
